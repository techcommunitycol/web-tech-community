...
export default function EventModalBody({ evt }: { evt: EventMeta }) {
  return (
    <>
      <div className="flex flex-wrap items-start justify-between gap-4 rounded-2xl border border-white/10 bg-white/[0.03] p-4 sm:p-5">
        <div className="min-w-0">
          <div className="mb-3 inline-flex rounded-full border border-primary/20 bg-primary/10 px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.18em] text-primary">
            Evento destacado
          </div>
          <h3 id="event-modal-title" className="text-xl font-semibold text-white sm:text-2xl">
            {evt.title}
          </h3>
      ...
      <div className="mt-6 grid gap-6 md:grid-cols-3">
        <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-4 sm:p-5 md:col-span-2">
          ...
          {evt.speakers && evt.speakers.length > 0 && (
            <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-4 sm:p-5">
              ...
              <ul className="mt-2 space-y-2">
                {evt.speakers.map((s) => (
                  <li key={s.name} className="flex items-center gap-3 rounded-xl border border-white/5 bg-black/10 px-3 py-2">
                    {s.avatar ? (
                    ...
                    {s.link && (
                      <a href={s.link} target="_blank" rel="noreferrer" className="ml-auto text-xs text-primary hover:text-cyan-200">
                        Perfil →
                      </a>
                    )}
                  </li>
            ...
            <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-4 sm:p-5">
              ...
              <div className="mt-2 flex flex-col gap-2">
                {evt.resources.slides && (
                  <a className="text-sm text-primary hover:text-cyan-200" href={evt.resources.slides} target="_blank" rel="noreferrer">
                    Ver slides →
                  </a>
                )}
                {evt.resources.repo && (
                  <a className="text-sm text-primary hover:text-cyan-200" href={evt.resources.repo} target="_blank" rel="noreferrer">
                    Repositorio →
                  </a>
                )}
                {evt.resources.recap && (
                  <a className="text-sm text-primary hover:text-cyan-200" href={evt.resources.recap} target="_blank" rel="noreferrer">
                    Post-recap →
                  </a>
                )}
              </div>
